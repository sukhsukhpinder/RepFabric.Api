...
namespace TemplateUploadLambda;
...
public class Function
{
    ...
    public Function()
    {
        ...
        _fileStorageService = services.BuildServiceProvider().GetRequiredService<IFileStorageService>();
    }
    ...
    {
        // Decode the file content from Base64 if necessary
        byte[] fileBytes;
        if (request.IsBase64Encoded)
        {
            fileBytes = Convert.FromBase64String(request.Body);
        }
        else
        {
            fileBytes = System.Text.Encoding.UTF8.GetBytes(request.Body);
        }

        using var fileStream = new MemoryStream(fileBytes);
        await _fileStorageService.SaveFileAsync(request.PathParameters["fileName"], fileStream);
        ...
        return new APIGatewayProxyResponse
        {
            ...
            Headers = new Dictionary<string, string>
            {
                ...
                { "Content-Disposition", "attachment; filename=\"filled-template.xlsm\"" }
            }
        };
    }
}
